import { describe, expect, it, vi, beforeEach } from "vitest";
import { fetchAllChats } from "./chat-cache.js";
import type { ResolvedRingCentralAccount } from "./accounts.js";
import * as api from "./api.js";

// Mock the API module
vi.mock("./api.js", () => ({
  listRingCentralChats: vi.fn(),
  getCurrentRingCentralUser: vi.fn(),
  getRingCentralUser: vi.fn(),
}));

const mockAccount: ResolvedRingCentralAccount = {
  accountId: "test",
  enabled: true,
  credentialSource: "config",
  clientId: "test-client-id",
  clientSecret: "test-client-secret",
  jwt: "test-jwt",
  server: "https://platform.ringcentral.com",
  config: {},
};

const mockLogger = {
  debug: vi.fn(),
  info: vi.fn(),
  warn: vi.fn(),
  error: vi.fn(),
};

describe("fetchAllChats Performance", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it("should measure time taken to resolve direct chats", async () => {
    // Setup: 5 direct chats needing resolution
    const chatCount = 5;
    const directChats = Array.from({ length: chatCount }, (_, i) => ({
      id: `chat-${i}`,
      type: "Direct",
      members: [{ id: "self-id" }, { id: `user-${i}` }],
    }));

    vi.mocked(api.getCurrentRingCentralUser).mockResolvedValue({ id: "self-id" } as any);

    vi.mocked(api.listRingCentralChats).mockImplementation(async ({ type }) => {
      if (type && type[0] === "Direct") {
        return directChats as any[];
      }
      return [];
    });

    vi.mocked(api.getRingCentralUser).mockResolvedValue({ firstName: "User", lastName: "Name" } as any);

    const start = Date.now();
    await fetchAllChats(mockAccount, mockLogger);
    const end = Date.now();
    const duration = end - start;

    console.log(`Duration for ${chatCount} chats: ${duration}ms`);

    // With optimization:
    // Batch size 3, delay 200ms.
    // 5 chats -> 2 batches.
    // Batch 1 (3 items): No delay.
    // Batch 2 (2 items): 200ms delay.
    // Total delay = 200ms.
    // We expect it to be much faster than 2000ms.
    // Ideally around 200-500ms depending on overhead.
    expect(duration).toBeLessThan(1000);
  }, 10000); // Increase timeout
});
