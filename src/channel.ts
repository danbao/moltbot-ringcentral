...
export const ringcentralDock: ChannelDock = {
  ...
  agentPrompt: {
    messageToolHints: () => [
      ...
      "- When user says '发给我' or 'send to me', the target should be the Direct chat between the bot and the sender, NOT the current group chat. Use `action=search-chat` with the sender's name to find their Direct chatId, then use that chatId for the action.",
    ],
  },
};
