...
export const ringcentralDock: ChannelDock = {
  ...
  agentPrompt: {
    messageToolHints: () => [
      ...
      "- When user says '发给我' or 'send to me', the target should be the sender's DM (Direct Message) chat with the bot, NOT the current group chat. The message context includes `senderId`; use `action=search-chat` with the sender's name to find the Direct chatId between the bot and that user, then use that Direct chatId for send/create-note/etc.",
    ],
  },
};
